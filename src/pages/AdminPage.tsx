...
const usuarios = [
  { id: 1, nombre: 'Ana García', email: 'ana@dulos.com', rol: 'ADMIN', activo: true },
  { id: 2, nombre: 'Carlos López', email: 'carlos@dulos.com', rol: 'OPERADOR', activo: true },
  { id: 3, nombre: 'María Torres', email: 'maria@dulos.com', rol: 'TAQUILLERO', activo: true },
  { id: 4, nombre: 'Pedro Ruiz', email: 'pedro@dulos.com', rol: 'OPERADOR', activo: false },
  { id: 5, nombre: 'Laura Díaz', email: 'laura@dulos.com', rol: 'TAQUILLERO', activo: true },
];
  ...
  return (
    <div>
      <div className="flex justify-end mb-4">
        <button
          onClick={() => alert('Invitar nuevo usuario')}
            ...
            <tr key={u.id} className="border-b">
              ...
              <td className="py-3">
                <span className="flex items-center gap-2">
                  <span
                    className={`w-2 h-2 rounded-full ${u.activo ? 'bg-green-500' : 'bg-gray-400'}`}
                  />
...
function RolesTab() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-3 gap-4">
      {roles.map((rol) => (
        <div key={rol.nombre} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
      ...
      <tbody>
        {logs.map((log, i) => (
          <tr key={i} className="border-b">
            <td className="py-3 text-gray-500 font-mono text-sm">{log.timestamp}</td>
            <td className="py-3">{log.usuario}</td>
            <td className="py-3">{log.accion}</td>
          </tr>
        ))}
      </tbody>
    </table>
